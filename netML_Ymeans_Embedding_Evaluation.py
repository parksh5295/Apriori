...
class YMeans:
    ...
    def fit_predict(self, X):
        ...
        kmeans = KMeans(n_clusters=best_k, max_iter=self.max_iter, tol=self.tol, random_state=self.random_state)
    ...
    def select_optimal_k(self, X):
        ...
        for k in k_range:
            kmeans = KMeans(n_clusters=k, max_iter=self.max_iter, tol=self.tol, random_state=self.random_state)
            kmeans.fit(X)
...
# Apply Y-means clustering
ymeans = YMeans(max_k=10)  # Try up to 10 clusters
print("Clustering in Progress...")

with tqdm(total=len(X_reduced), desc="Clustering", unit="samples") as pbar:
    cluster_labels, best_k = ymeans.fit_predict(X_reduced)
...
# Save Results to CSV
data[['cluster', 'adjusted_cluster', 'label']].to_csv("./netML_Ymeans_clustering_Compare.csv", index=False)
metrics_df = pd.DataFrame([metrics_original, metrics_adjusted], index=["Original", "Adjusted"])
metrics_df.to_csv("./netML_Ymeans_clustering_Compare_Metrics.csv", index=True)
