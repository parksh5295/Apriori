...
X_reduced = pca.fit_transform(X).T  # Transpose for skfuzzy

# 6. Apply Custafson-Kessel (FCM-based) Clustering
num_clusters = 2  # Assuming binary classification
cntr, u, _, _, _, _, _ = fuzz.cluster.cmeans(X_reduced, num_clusters, 2, error=0.005, maxiter=1000, init=None)

# Assign cluster labels with progress bar
print("Clustering in Progress...")
with tqdm(total=len(X_reduced.T), desc="Clustering", unit="samples") as pbar:
    cluster_labels = np.argmax(u, axis=0)
    data['cluster'] = cluster_labels
    pbar.update(len(X_reduced.T))  # Update when clustering is complete
...
def evaluate_clustering(y_true, y_pred, X_data):
    if not y_true.empty:
        return {
            ...
            "macro_silhouette": silhouette_score(X_data.T, y_pred) if len(set(y_pred)) > 1 else np.nan,
            ...
            "micro_silhouette": silhouette_score(X_data.T, y_pred) if len(set(y_pred)) > 1 else np.nan,
            ...
            "weighted_silhouette": silhouette_score(X_data.T, y_pred) if len(set(y_pred)) > 1 else np.nan,
        }
    return {}
...
# Save Results to CSV
data[['cluster', 'adjusted_cluster', 'Label']].to_csv("./MitM_CK_clustering_Compare.csv", index=False)
metrics_df = pd.DataFrame([metrics_original, metrics_adjusted], index=["Original", "Adjusted"])
metrics_df.to_csv("./MitM_CK_clustering_Compare_Metrics.csv", index=True)
