...
# 6. Apply Growing Neural Gas (NeuralGas) Clustering
num_clusters = 2  # Assuming binary classification
gnn = GrowingNeuralGas(n_neurons=num_clusters, max_iter=1000, verbose=True)
gnn.fit(X_reduced)

# Get the closest neuron for each data point (cluster assignment)
cluster_labels = gnn.predict(X_reduced)
...
# Save Results to CSV
data[['cluster', 'adjusted_cluster', 'Label']].to_csv("./MitM_NeuralGas_clustering_Compare.csv", index=False)
metrics_df = pd.DataFrame([metrics_original, metrics_adjusted], index=["Original", "Adjusted"])
metrics_df.to_csv("./MitM_NeuralGas_clustering_Compare_Metrics.csv", index=True)
