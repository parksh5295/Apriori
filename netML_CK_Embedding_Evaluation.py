...
# Custafson-Kessel Algorithm using skfuzzy
def custafson_kessel(X, c, m=2, maxiter=1000, error=1e-5):
    n_samples, n_features = X.shape
    u = np.random.rand(c, n_samples)  # Membership matrix
    u = u / np.sum(u, axis=0)  # Normalize the membership matrix
    cntr = np.zeros((c, n_features))  # Cluster centers
    pbar = tqdm(total=maxiter, desc="CK Algorithm", unit="iteration")
    
    # Calculate initial cluster centers using FCM (Fuzzy C-means)
    cntr, u, _, _, _, _ = fuzz.cmeans(X.T, c, m, error, maxiter)
    
    for iteration in range(maxiter):
        # Calculate the fuzzy covariance matrix for each cluster
        fuzzy_covariances = np.zeros((c, n_features, n_features))
        for i in range(c):
            dist = np.linalg.norm(X - cntr[i], axis=1)
            # Compute covariance matrix for each cluster
            fuzzy_covariances[i] = np.cov(X.T, aweights=u[i, :]**m)
        
        # Calculate new cluster centers considering the covariance matrices
        for i in range(c):
            cntr[i] = np.sum((u[i, :]**m).reshape(-1, 1) * X, axis=0) / np.sum(u[i, :]**m)
    ...
    pbar.close()
    return cntr, u  # Returning cluster centers and membership matrix
...
# Apply Custafson-Kessel (CK) clustering
n_clusters = 2  # Set number of clusters (in your case 2)
cntr, u = custafson_kessel(X_reduced, n_clusters)

# Determine the cluster labels
cluster_labels = np.argmax(u, axis=0)  # Assign each sample to the cluster with the highest membership value
...
# Save Results to CSV
data[['cluster', 'adjusted_cluster', 'label']].to_csv("./netML_CK_clustering_Compare.csv", index=False)
metrics_df = pd.DataFrame([metrics_original, metrics_adjusted], index=["Original", "Adjusted"])
metrics_df.to_csv("./netML_CK_clustering_Compare_Metrics.csv", index=True)
