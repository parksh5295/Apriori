...
dbscan = DBSCAN(eps=0.5, min_samples=5)  # Set eps and min_samples based on your data
...
# DBSCAN assigns -1 to noise points, which are not part of any cluster. If needed, you can adjust them.
data['adjusted_cluster'] = data['cluster'].apply(lambda x: x if x != -1 else -1)  # Adjusting noise points as -1
...
# Save Results to CSV
data[['cluster', 'adjusted_cluster', 'label']].to_csv("./netML_DBSCAN_clustering_Compare.csv", index=False)
metrics_df = pd.DataFrame([metrics_original, metrics_adjusted], index=["Original", "Adjusted"])
metrics_df.to_csv("./netML_DBSCAN_clustering_Compare_Metrics.csv", index=True)
