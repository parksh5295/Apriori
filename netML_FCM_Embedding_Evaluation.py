...
n_clusters = 2  # Set the number of clusters (in your case 2)

# Perform Fuzzy C-Means clustering with progress bar
with tqdm(total=1000, desc="Fuzzy C-Means Clustering", unit="iteration") as pbar:
    # Perform Fuzzy C-Means clustering
    cntr, u, _, _, _, _, _ = fuzz.cmeans(X_reduced.T, c=n_clusters, m=2, error=0.005, maxiter=1000, init=None)
    
    # Simulating the progress bar update (here for maxiter=1000 iterations)
    for _ in range(1000):
        pbar.update(1)
...
# Save Results to CSV
data[['cluster', 'adjusted_cluster', 'label']].to_csv("./netML_FCM_clustering_Compare.csv", index=False)
metrics_df = pd.DataFrame([metrics_original, metrics_adjusted], index=["Original", "Adjusted"])
metrics_df.to_csv("./netML_FCM_clustering_Compare_Metrics.csv", index=True)
