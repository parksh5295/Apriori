...
# 6. Apply Fuzzy C-means (FCM) Clustering
...
# Save Results to CSV
data[['cluster', 'adjusted_cluster', 'Label']].to_csv("./MitM_FCM_clustering_Compare.csv", index=False)
metrics_df = pd.DataFrame([metrics_original, metrics_adjusted], index=["Original", "Adjusted"])
metrics_df.to_csv("./MitM_FCM_clustering_Compare_Metrics.csv", index=True)
