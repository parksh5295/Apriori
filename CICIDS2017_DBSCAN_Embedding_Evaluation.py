...
dbscan = DBSCAN(eps=0.5, min_samples=5)  # eps: neighborhood size, min_samples: minimum points to form a cluster
...
# Save Results to CSV
data[['cluster', 'label']].to_csv("./CICIDS2017_DBSCAN_clustering_Compare.csv", index=False)
metrics_df = pd.DataFrame([metrics_original], index=["Original"])
metrics_df.to_csv("./CICIDS2017_DBSCAN_clustering_Compare_Metrics.csv", index=True)
