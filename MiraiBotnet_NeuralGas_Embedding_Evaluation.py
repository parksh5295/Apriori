...
neural_gas = GrowingNeuralGas(n_inputs = X_reduced.shape[1], n_start_nodes=2, max_nodes=50, step=0.2, max_edge_age=50)
neural_gas.train(X_reduced, epochs=100)
...
labels = np.array([assign_cluster(x, neural_gas.graph) for x in X_reduced])
data['cluster'] = labels
...
if len(np.unique(labels)) > 1:
    ...
    nmi = normalized_mutual_info_score(data['label'], labels)
else:
    silhouette = ari = nmi = np.nan

# Save results to CSV
data[['cluster', 'label']].to_csv("./MiraiBotnet_NeuralGas_clustering_Results.csv", index=False)   # silhouette, ARI, NMI
...
# Save results to CSV
data[['cluster', 'adjusted_cluster', 'label']].to_csv("./MiraiBotnet_NeuralGas_clustering_Compare.csv", index=False)
...
metrics_df.to_csv("./MiraiBotnet_NeuralGas_clustering_Compare_Metrics.csv", index=False)
