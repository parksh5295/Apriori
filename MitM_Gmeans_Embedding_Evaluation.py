...
X_reduced = pca.fit_transform(X).T  # Transpose for G-means

# 6. Apply G-means Clustering using KMeans + Gaussian Mixture Model
def gmeans(X, max_clusters=10):
    X = X.T  # Transpose so each sample is a row
    km = KMeans(n_clusters=2, random_state=42)
    km.fit(X)
    labels = km.labels_
    
    # Check Gaussian Mixture Model (GMM) fit for each cluster
    gm = GaussianMixture(n_components=2, covariance_type='full', random_state=42)
    gm.fit(X)
    
    # GMM should give probabilities of belonging to clusters
    pred_labels = gm.predict(X)
    
    return pred_labels

# Apply G-means clustering
cluster_labels = gmeans(X_reduced)
...
# Save Results to CSV
data[['cluster', 'adjusted_cluster', 'Label']].to_csv("./MitM_Gmeans_clustering_Compare.csv", index=False)
metrics_df = pd.DataFrame([metrics_original, metrics_adjusted], index=["Original", "Adjusted"])
metrics_df.to_csv("./MitM_Gmeans_clustering_Compare_Metrics.csv", index=True)
