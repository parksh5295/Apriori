...
class GMMCluster:
    ...
    def fit_predict(self, X):
        ...
        gmm = GaussianMixture(n_components=best_k, max_iter=self.max_iter, tol=self.tol, random_state=self.random_state)
    ...
    def select_optimal_k(self, X):
        ...
        for k in k_range:
            gmm = GaussianMixture(n_components=k, max_iter=self.max_iter, tol=self.tol, random_state=self.random_state)
            gmm.fit(X)
...
gmm_cluster = GMMCluster(max_k=10)  # Try up to 10 clusters
print("Clustering in Progress...")

with tqdm(total=len(X_reduced), desc="Clustering", unit="samples") as pbar:
    cluster_labels, best_k = gmm_cluster.fit_predict(X_reduced)
...
# Save Results to CSV
data[['cluster', 'adjusted_cluster', 'label']].to_csv("./netML_GMM_clustering_Compare.csv", index=False)
metrics_df = pd.DataFrame([metrics_original, metrics_adjusted], index=["Original", "Adjusted"])
metrics_df.to_csv("./netML_GMM_clustering_Compare_Metrics.csv", index=True)
