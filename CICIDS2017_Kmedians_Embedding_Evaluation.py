...
# Apply K-Medians clustering (using KMedoids with Manhattan distance)
n_clusters = 2  # 클러스터 개수 설정
kmedians = KMedoids(n_clusters=n_clusters, metric='manhattan', random_state=42)
print("Clustering in Progress...")

with tqdm(total=len(X_reduced), desc="Clustering", unit="samples") as pbar:
    cluster_labels = kmedians.fit_predict(X_reduced)
...
# Save Results to CSV
data[['cluster', 'label']].to_csv("./CICIDS2017_Kmedians_clustering_Compare.csv", index=False)
metrics_df = pd.DataFrame([metrics_original], index=["Original"])
metrics_df.to_csv("./CICIDS2017_Kmedians_clustering_Compare_Metrics.csv", index=True)
