...
# Save results to CSV
data[['cluster', 'adjusted_cluster', 'label']].to_csv("./MiraiBotnet_Kmeans_clustering_Compare_v.2.csv", index=False)

# Save metrics to CSV
metrics = {
    'Metric': ['Accuracy', 'Precision', 'Recall', 'F1 Score', 'Jaccard Index'],
    'Original': [accuracy, precision, recall, f1, jaccard],
    'Adjusted': [accuracy_adjusted, precision_adjusted, recall_adjusted, f1_adjusted, jaccard_adjusted]
}
metrics_df = pd.DataFrame(metrics)
metrics_df.to_csv("./MiraiBotnet_Kmeans_clustering_Compare_Metrics_v.2.csv", index=False)
