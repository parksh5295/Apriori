...
# Apply Custafson-Kessel clustering (Similar to Fuzzy C-Means)
n_clusters = 2  # Number of clusters
m = 2  # Fuzziness parameter (similar to Fuzzy C-Means)
max_iter = 1000  # Max iterations
error_threshold = 0.005  # Error threshold

# Fuzzy C-means initialization
fcm_model = fuzz.cluster.cmeans(X_reduced.T, c=n_clusters, m=m, error=error_threshold, maxiter=max_iter, init=None)

# Update: Custafson-Kessel might modify centroids and fuzzy matrix with geometric considerations, not just Euclidean.
# But here, we use a similar structure for the algorithm.
with tqdm(total=max_iter, desc="Custafson-Kessel Clustering", unit="iteration") as pbar:
    for i in range(fcm_model[0].shape[1]):  # Loop through all iterations
        fcm_model = fuzz.cluster.cmeans(X_reduced.T, c=n_clusters, m=m, error=error_threshold, maxiter=1, init=fcm_model[0])
        pbar.update(1)

# Fuzzy partition matrix (U) is used to assign the cluster with the highest membership
...
# Save Results to CSV
data[['cluster', 'label']].to_csv("./CICIDS2017_CK_clustering_Compare.csv", index=False)
metrics_df = pd.DataFrame([metrics_original], index=["Original"])
metrics_df.to_csv("./CICIDS2017_CK_clustering_Compare_Metrics.csv", index=True)
