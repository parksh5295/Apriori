...
# Embedding method is similar with K-means ver.1
...
metrics = pd.DataFrame([
    {"Metric": key, "Original": val, "Adjusted": metrics_adjusted[key]}
    for key, val in metrics_original.items()
])
metrics.to_csv("./MiraiBotnet_Ymeans_clustering_Compare_Metrics.csv", index=False)

# Save results to CSV
data[['cluster', 'adjusted_cluster', 'label']].to_csv("./MiraiBotnet_Ymeans_clustering_Compare.csv", index=False)
