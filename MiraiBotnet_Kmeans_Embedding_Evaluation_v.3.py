...
# kmeans = KMeans(n_clusters=2, random_state=42)    # Clustering initialization is the same every time
kmeans = KMeans(n_clusters=2, random_state=None, n_init=50)
# data['cluster'] = kmeans.fit_predict(X_reduced)   # PCA ver
...
if not data_filtered.empty:
    ...
    ma_silhouette = silhouette_score(X[data['cluster'] != -1], data_filtered['cluster'])
else:
...
if not data_filtered_adjusted.empty:
    ...
    ma_silhouette_adjusted = silhouette_score(X[data['adjusted_cluster'] != -1], data_filtered_adjusted['adjusted_cluster'])
else:
...
if not data_filtered.empty:
    ...
    mi_silhouette = silhouette_score(X[data['cluster'] != -1], data_filtered['cluster'])
else:
...
if not data_filtered_adjusted.empty:
    ...
    mi_silhouette_adjusted = silhouette_score(X[data['adjusted_cluster'] != -1], data_filtered_adjusted['adjusted_cluster'])
else:
...
if not data_filtered.empty:
    ...
    w_silhouette = silhouette_score(X[data['cluster'] != -1], data_filtered['cluster'])
else:
...
if not data_filtered_adjusted.empty:
    ...
    w_silhouette_adjusted = silhouette_score(X[data['adjusted_cluster'] != -1], data_filtered_adjusted['adjusted_cluster'])
else:
    w_accuracy_adjusted = w_precision_adjusted = w_recall_adjusted = w_f1_adjusted = w_jaccard_adjusted = w_silhouette_adjusted = np.nan

# Save results to CSV
data[['cluster', 'adjusted_cluster', 'label']].to_csv("./MiraiBotnet_Kmeans_clustering_Compare_v.3.csv", index=False)
...
metrics_df.to_csv("./MiraiBotnet_Kmeans_clustering_Compare_Metrics_v.3.csv", index=False)
