...
# Apply Spherical GMM (Gaussian Mixture Model with spherical covariance)
n_clusters = 2  # 클러스터 개수 설정
spherical_gmm = GaussianMixture(n_components=n_clusters, covariance_type='spherical', random_state=42)
print("Clustering in Progress...")

with tqdm(total=len(X_reduced), desc="Clustering", unit="samples") as pbar:
    cluster_labels = spherical_gmm.fit_predict(X_reduced)
...
# Save Results to CSV
data[['cluster', 'label']].to_csv("./CICIDS2017_SGMM_clustering_Compare.csv", index=False)
metrics_df = pd.DataFrame([metrics_original], index=["Original"])
metrics_df.to_csv("./CICIDS2017_SGMM_clustering_Compare_Metrics.csv", index=True)
